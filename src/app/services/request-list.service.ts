...
export class RequestListService{
    ...
    getRequestLists(url: string) : Observable<RequestDetail[]> {
        return this.http.get(url).map(data=>{
            ...
            if (roleList instanceof Array) {
                results = roleList.map(function (request: any) {
                    return {creation_date: request.creation_date,
                            role: request.role,
  ...
  getRequestList(url: string) : Observable<RequestDetail[]> {
    return this.http.get(url).map(data=>{
      let roleList = data;
      if (roleList instanceof Array) {
        return roleList.map(function (request: any) {
          return {creation_date: request.creation_date, role: request.role, employee: request.employee,
            status: request.status, url: request.url, id: request.id};
        });
      }
    });
  }
